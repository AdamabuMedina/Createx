...
if (portSlider) {
  ...
  document.querySelector(".portfolio-section__prev").addEventListener("click", () => {
    ...
    document.querySelectorAll(".portfolio-section__items .swiper-slide").forEach(element => {
      element.classList.remove("slider-visible")
    });
...
if (relatedSlider) {
  ...
  document.querySelector(".related-projects__prev").addEventListener("click", () => {
    ...
    document.querySelectorAll(".related-projects__items .swiper-slide").forEach(element => {
      element.classList.remove("slider-visible")
    });
...
if (historySlider) {
  const historyImages = new Swiper(historySlider, {
    spaceBetween: 20,
    slidesPerView: 1,
    navigation: {
  ...
  historyImages.on("slideChange", function () {
    ...
    document.querySelector(`.history-nav__btn[data-index="${historyImages.realIndex}"`).classList.add("history-nav__btn--active")
  })
